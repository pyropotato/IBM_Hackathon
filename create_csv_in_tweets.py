...
def extract_in_tweets(df, file_, words_list):
    df_no_code = df.loc[((df['country_code'] == 'IN')| df['country_code'].isnull()) & (df['lang'] == 'en')][['user_id', 'created_at', 'screen_name','text', 'retweet_count', 'followers_count', 'favourites_count','verified']]
